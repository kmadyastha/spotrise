...
export async function POST(request: Request) {
  try {
    ...
    const prompt = `You are a local-business consultant drafting the Q&A section for "${business.name}"'s Google Business Profile. Here are real customer reviews for context:
${reviewTexts.slice(0, 30).map((t, i) => `${i + 1}. "${t}"`).join("\n") || "(no review text available)"}

Write 6 realistic customer questions this business is likely to get, with concise draft answers. Respond with ONLY valid JSON (no markdown fences, no commentary):
{ "qas": [ { "question": "<realistic customer question>", "answer": "<concise draft answer, under 300 characters>" } ] }

Cover practical topics customers actually ask about for a business like this (parking, walk-ins vs appointment, payment methods, what first-timers should know, accessibility) — infer only what's reasonably implied by the reviews above. Critically: you do not actually know this business's exact hours, prices, or other specific facts not stated in the reviews — for any answer that would need one, write it as a clearly-labeled placeholder instead of inventing a number, e.g. "[Add your hours here]" or "[Confirm current pricing]". Never fabricate a specific fact that isn't backed by the reviews.`;
    ...
    await supabase.from("tool_usage").insert({ user_id: user.id, business_id: businessId, tool: "qa_generator" });

    return NextResponse.json({ qas: parsed.qas ?? [] });
  } catch (err) {
    console.error("qa-generator failed:", err);
